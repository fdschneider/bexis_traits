...
![Elements of the original data set (species $\times$ trait matrix) are transferred into a observation table format with additional columns for trait and taxon standardization. Further information on the level of measurements, occurrences (i.e. individual specimens) and sampling locations can be added via the extended vocabulary.](figures.PNG)
...
This creates a clean text file (i.e. it omits the row names when producing a comma separated file with points as decimal delimiter) ready for upload on BExIS. 
...
**On page 6:** Set 'exploratory' and 'keyPlots' to 'none' if your trait data has no column with specific exploratories information, otherwise enter the column that contains the plot ID (by default "ExploratoriesPlotID"). If you created a .txt or .csv file from the R package, choose fileFormat 'ASCII' and double check for the correct field delimiter (usually ",") and decimal separator (usually "."). Structural missing values for empty columns created by the R package are empty fields, thus enter "". 
Known missing values (i.e. trait values which have not been measured for any reason) should not exist in the observation table format (the R package eliminates them by default, if they were correctly marked as NA in the original data). If you choose to keep rows for lacking trait measurements in the table, they should be marked as "NA". The section on 'matrix data' can be ignored since the data have been transferred into observation table format. The variables (i.e. column names) are already provided by the template. You may add non-standard columns if necessary. 
...
- For troubleshooting during file upload, please contact BExIS Helpdesk (mail@?.de). For issues with the definitions of the vocabulary, please get involved in the discussion on Github (https://github.com/EcologicalTraitData/ETS/issues). 
