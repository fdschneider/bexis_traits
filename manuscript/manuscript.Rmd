...
We discuss these initiatives seperately although often they are developed in conjunction to serve a particular database project, as for instance in the case of the TRY plant database [@kattge11; @kattge11a] and the TOP Thesaurus of plant traits [@garnier17]. 
...
Regarding open access, hardly any of these centralized databases for functional traits comply with the criteria demanded by journals and funding agencies for primary data publication. They incentivize data submissions by offering increased data visibility and usage, while providing data use policies that secure author attribution and, if demanded by the data owner, co-authorship.  
...
Specifically designed for the context of plant traits, @kattge11 proposed a generic structure for plant trait data, that covers most potential use cases. This data structure is build around a central data table that contains observations, i.e. a single event of measurements on the same individual plant specimen at the same point in time. Identifiers link multiple measurements to the same observation, each measurement being well defined by additional standard tables. The observations are also linked to a taxonomy and ancillary descriptors of the observation context, like location or experimental treatment. This structure can be implemented in any software or programming language for relational databases. 
...
However, these proposed standards and the available terms and extensions are responses to a demand from biodiversity data management for more structured input from the research community. 
None of the aforementioned aggregated and harmonized databases for birds, amphibians, or mammals employs such ontologies or semantic web standards. 
...
<!-- PM: There is also little incentive for researchers to do this- why bother? Incentives are also needed. -->
The pay-off is only playing out in the long run. Citations of trait datasets are higher when they are easier to access and relate to other data due to standard structures and vocabularies. 
...
As a response to the observations and conclusions above, we propose a versatile vocabulary that captures the different degrees measurement detail for multiple use cases of trait-based ecological research.
...
- we propose Vocabulary for two-dimensional tables of traits. 
- long-table format. has advantage of easy merging and filtering, computing, modelling. But disadvantage of multi-type columns (e.g. for values) and repetitive content. For complex dimensional databases, use Kattge, DwC or ABCD instead, packaged as ZIP files with annotations in EML Metadata format rather than multi-spreadsheet files. 
- linking to standard taxonomy and trait terminology. Allowing for but minimizing duplicate information.  by providing standardised duplicates, align user-provided nomenclature and trait methodology with taxonomic nomenclature and trait ontologies.  transparency for data user. 
...
We paid particular consideration to the work of @kattge11, @kattge11a,  and @garnier17, as well as @parr16 to ensure compatibility of our proposed data structure with major trait databases and existing standards for biodiversity data management. <!-- AO: Webservices!! ;  Really to be considered as standards? -->
...
However, two-dimensional spreadsheets are limited in the number and complexity of measurements they can contain. 
In more complex datasets, the repetition of verbose entries and long character strings in a single data table should be avoided. Therefore we recommend use relational datatabase structures composed of multiple spreadsheets linked via identifiers, like for example the generic trait database structure proposed by @kattge11a or the Traitbank structure proposed by @parr15, ideally compiled into Darwin Core Archives [DwC-A, @robertson09]. 
...
To serve the demand for simple ways to standardise and harmonize ecological trait data, we propose a versatile vocabulary for two-dimensional datasets.  
...
Thanks to all respondents to an internal online survey on trait data for the Biodiversity Exploratories project and to Matthias Biber, Diana Bowler, Klaus Birkhofer, Runa Boeddinghaus, Martin Gossner, Catrin Westphal and Jens Kattge for comments on the manuscript, the trait data standard and the R-package. 
