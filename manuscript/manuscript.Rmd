...
- name: Gaetane Le Provost \footnotemark[2]
- name: Andreas Ostrowski \footnotemark[5]
- name: Caterina Penone \footnotemark[4]
- name: Markus Fischer \footnotemark[4]\footnotemark[2]
- name: Pete Manning \footnotemark[2]
- name: Birgitta König-Ries\footnotemark[5]
...
3. In order to facilitate the standardisation and harmonization of distributed trait datasets, we propose a general and simple vocabulary and a two-dimensional data structure for storing and sharing ecological trait data. 
4. We provide an R-package that enables the transformation of any data into the proposed form. This also allows to harmonize and merge trait datasets from heterogeneous sources, which facilitates the compilation of databses of particular research focus. 
5. With these decentralized tools for trait-data harmonization, we intend to facilitate the exchange and analysis of trait data for ecological research and enable a global synthesis of traits across a wide range of organisms and ecosystems. 
...
Trait-based studies have spanned the evolutionary basis of individual-level properties (refs) to global patterns of biodiversity [@diaz16] and ecosystem functioning (refs). 
For example, using traits is a promising venue to bypass taxonomic impediment, i.e. the fact that a majority of species are yet undescribed and little is known of their interactions with the environment and other organisms (refs), since they allow inferring the ecological role of organisms [@duarte11; @schrodt15] from their apparent features.
...
However, fully exploiting the potential of trait-based approaches relies heavily on the broad availability and taxonomic and regional coverage of trait data, both of present-day taxa ase well as in evolutionary deep-time. Researchers have a growing interest in assembling trait data from different sources since it pays off by getting answers to general questions. 
...
To this end, trait data have been standardized in centralized databases for specific organism groups and regional scope, often centered around specific research questions (ref to TRY, panthera, etc.). 
These compiled data are either being published as static data tables that are the product of a one-time effort of collecting and harmonizing data (ref Amphibio, panthera, mammals), or in the form of sophisticated online portals, which attract data submissions from a focal community of researchers and provide research networks for consensus building in data resolution and measurement methods [e.g. @kattge11]. Furthermore, these portals map heterogeneous data into a common scheme, and importantly also, offer access control and data usage policies, to protect the rights of the original data providers, while simplifying data queries for synthesis researchers who seek to utilize these often-complex datasets. 
To enable the compatibility of data across data-bases, semantic-web standards [@wieczorek12; @page08] and ontologies of standard terms [@walls12; @garnier17] for trait-data are beeing developed.
Solving all these issues of data assemblage and harmonization in centralised infrastructures requires a huge effort in costs and personell (see below for a review of current initiatives). 

<!-- 4. distributed trait data must be integrated --> 
Many journals and research funding agencies today require data to be published without access restrictions [@swan12; @allison15]. An increasing number of individual projects comply  with open data standards and publish their primary data on file hosting services like Figshare, Dryad or Researchgate, where no data standards are forced upon the uploaded material. The data standards mentioned above, that have been proposed for relational trait-databases, do hardly apply for these individual datasets.
...
In this paper, we (1.) review existing trait databases and online portals, as well as initiatives for standardisation. In this review, we discuss the current practice and the importance of data standards for trait-based research and identify current deficits in standardisation from a pragmatic view of data providers and data users. Based on these considerations, we (2.) propose a minimal structure and vocabulary for describing trait-datasets, compatible with existing terminology standards for biodiversity data. Finally, we (3.)  propose the development of software that assists in aplying a harmonisation of data from distributed sources. 

With this easy-to-use terminology and toolset, we hope to inform trait-data providers and trait-data users about the general importance of data standardisation and sketch a roadmap towards an accessible trait data standardisation.
...
- The first are projects that work with trait datasets which have been assembled out of a particular research interest, either by measurement or collated from literature; 
...
- the third aim at the standardization and develops consensus methods and definitions for traits and provides standard terminologies; 
- the fourth kind of initiative feed the needs of biodiversity informatics and provide database structures and relational definitions to construct relational 'ontologies' for an application in a 'semantic web' of information. 

We discuss these initiatives seperately although often they are developed in conjunction to serve a particular database project, as for instance in the case of the TRY plant database [@kattge11; @kattge11a] and the  Thesaurus of Plant Traits [TOP; @garnier17]. 
We show how the degree of trait-data standardization in exising datasets spans along this entire spectrum and which tools and standards do exist to acheive harmonization of data from multiple, distributed sources. 
The objective of this review is to raise awareness for the generic structure of trait data and aid ecological researchers to share and publish own datasets in the appropriate form. 
...
Today, the online publication of such data is greatly facilitated by hosting services like  Figshare.com, Dryad (datadryad.org), Researchgate.net or Zenodo.org. 
...
In the past two decades,  distributed trait data have been aggregated and harmonized into greater collections with particular taxonomic or regional focus [e.g. @klotz02; @iversen17; @kleyer08; @oliveira17; @iversen17; @myhrvold15; @kissling14; @jones09]. <!--[@wright04; ] -->
...
Regarding open access, hardly any of these centralised databases for traits comply with the criteria demanded by journals and funding agencies for primary data publication. They incentivise data submissions by offering increased data visibility and usage, while providing data use policies that secure author attribution and, if demanded by the data owner, co-authorship.  
Time will show how these incentives fare with an increasing availability of open access trait data  on low-threshold data repositories. 
...
By filling this gap, data brokering services (e.g. the German Federation on for Biological Data; gfbio.org) or data management systems for scientific projects (e.g. BExIS, NEON<!--BKR: Diversity Workbench (developed at SNSB), Metacat (https://knb.ecoinformatics.org/)--> ) are likely to gain importance. These services simplify and direct the standardized upload of research data and descriptive metadata into reliable and interlinked data infrastructures. One goal of such initiatives is to facilitate data publications and standardization for researchers, for instance by providing terminologies and ontologies for biodiversity data, and by consulting on publication licenses. <!-- AG: Other examples: Diversity Workbench (diversityworkbench.net), EDIT Platform for Cybertaxonomy (cybertaxonomy) -->
...
Rather than describing a data structure for relational databases, this is intended as a more inclusive terminology, that can be used in simple two-dimensional datasets as well as in the exchange of data between web services in a semantic web.  
...
We paid particular consideration to the work of @kattge11, @kattge11a,  and @garnier17, as well as @parr16 to ensure compatibility of our proposed data structure with major trait databases and existing standards for biodiversity data management. 
...
- Offering compatibility, input for dimensional databases by using global identifiers and internal identifiers that provide hierarchical layers (measurement, occurrence, sampling event, data source, bibliographic reference)
- using these identifiers enables the vocabulary for for observation level data (linked via an occurrence identifier) raw measurement data (each row an own measurement identifier), as well as for aggregated data on the species level (each row an own measurement identifier plus aggregation information, like taxon rank or statistical method). allows for originary data (reporting own measuremetns) or compiled data sheets (reporting multiple dataset identifiers).  
- wherever appropriate, use URIs for linking to external ressources. 
...
- long-table format. has advantage of easy merging and filtering, computing, modelling. But disadvantage of multi-type columns (e.g. for values) and repetitive content. 

- linking to standard taxonomy and trait terminology. semantic web standards, URI identifiers of external resources. 

- Allowing for but minimizing duplicate information.  by providing standardised duplicates, align user-provided nomenclature and trait methodology with taxonomic nomenclature and trait ontologies.  transparency for data user. 

- vocabulary is structured into sections: core, measurement, occurrence, metadata, traitlists. Short definitions. These sections are meant to guide through the vocabulary. their structural meaning depends on context provided by hierarchical IDs. 

Two-dimensional spreadsheets are limited in the number and complexity of measurements they can contain. For datasets containing multi-layered information on observations, traits, taxa and environmental context, we highly recommend the use relational datatabase structures composed of multiple spreadsheets linked via identifiers, like for example the generic trait database structure proposed by @kattge11a or the Traitbank structure proposed by @parr15, ideally compiled into Darwin Core Archives [DwC-A, @robertson09]. 
...
<!-- 
(We conceptualize trait-datasets to  not include information about a species' interaction partners/associations of an organism, since they are -- to our understanding -- not within the scope of the Entity-Quality Model. However, such network data may be of interest to be combined with trait data. ) <!-- Malte: which can only be achieved if both types of datasets have been related to an ontology of species names. -->
...
In the case of raw measurement data, this information applies to the entire trait dataset, and should be stored along with the published data as metadata (e.g. in a separate file or on the download website). 
...
Many terms of these extensions refine or copy terms of the Darwin Core Standard and their own [Taxon](http://rs.tdwg.org/dwc/terms/index.htm#Taxon),  [MeasurementOrFact](http://rs.tdwg.org/dwc/terms/index.htm#measureindex) and [Occurrence](http://rs.tdwg.org/dwc/terms/index.htm#Occurrence) extensions and EOL TraitBank's use of those terms (http://eol.org/info/structured_data_archives).
...
The online reference for the Ecological Traitdata Standard Vocabulary described in this paper is https://ecologicaltraitdata.github.io/ETS/
