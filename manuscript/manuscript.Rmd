...
3. In order to facilitate the standardisation and harmonisation of distributed trait datasets on the data-provider side, we propose a standardised vocabulary to be used for storing and sharing ecological trait data. We discuss impediments and incentives for a wide adoption of such a standard by data-providers. 
4. The use of a standard vocabulary allows trait datasets from heterogeneous sources to be compiled more easily into databases, and facilitates the creation of interfaces between software tools for trait-data handling and analysis. By aiding decentralised trait-data standardisation, our data standard facilitates the use of trait data for a broader ecological research community and enables global syntheses across a wide range of taxa and ecosystems. 
...
Recent years have seen a proliferation of trait-based research in a wide range of fields. 
Traits bridge the evolutionary basis of individual-level properties [@salguero-gomez16] to global patterns of biodiversity [@diaz16] and relate losses of ecosystem function to changes in  the functional composition of species assemblages [@mouillot13; @perovic15; @bello10]. This offers the mechanistic background for linking biodiversity to climate change or local anthropogenic land use [@diaz11; @lavorel12; @allan15].
...
To enable such functional ecological research, trait data have been harmonized and compiled into centralised databases for specific organism groups and regional scope, often centred around particular research questions [e.g. PanTHERIA, @jones09; TRY, @kattge11; AmphiBio, @oliveira17]. 
The heterogeneity of data from different research contexts <!-- i.e. differences in taxonomic nomenclature and resolution (e.g. reported on species level or aggregated on higher taxonomical orders), the scale and place of the study context, or the accurracy of the methodology applied in measurements--> render trait data extremely heterogeneous and make the task of data compilation time-consuming and error-prone.
Besides initiatives aiming at assembling data, other tools to enable the compatibility of data across databases are being developed. These include software to access trait data from the internet [e.g. @chamberlain17; @ankenbrand18], semantic-web standards [@wieczorek12; @page08] and ontologies of standard terms [@walls12; @garnier17].

<!-- 4. distributed trait data must be integrated --> 
Meanwhile, open science reaches the mainstream: publication of research data without access restrictions, with structured metadata and in accordance to data standards to enable their reuse, has become the declared goal of an open biodiversity knowledge management (http://www.bouchoutdeclaration.org/) and is increasingly demanded by journals and public research funding [@emerson15; @allianceofgermanscienceorganisations10; @royalsocietysciencepolicycentre12; @swan12; @allison15]. As a result, an increasing number of individual research projects publish their primary data on general-purpose file hosting services (e.g. Figshare.com, datadryad.org, Researchgate.net, or Zenodo.org), where no data standards are forced upon the uploaded material [@wilkinson16]. 
...
In this paper, we review existing trait databases and online portals, as well as initiatives for data standardisation from a pragmatic view of data providers and data users. We discuss the current efforts for making trait data visible, accessible and reuseable in downstream data analysis, as demanded by the FAIR principles for scientific data [@wilkinson16]. We show how the current deficit in standardisation hampers the implementation of interoperability and reuse of trait data. 
...
Since this standard vocabulary relies on the adoption of a broad research community we discuss incentives for its use and lay out mechanisms for a community-based future development towards an accessible ecological trait data standard.
...
We review four types of initiatives that are of relevance for trait-data standardisation (see Glossary in Table 1 for italicised terms):
...
Today, the online publication of such data is greatly facilitated by *file hosting services* (e.g. Figshare.com, datadryad.org, Researchgate.net, or Zenodo.org), which warrant long-term accessibility and citability via DOIs, and support Public Domain dedication or Creative Commons licenses.
...
Most importantly, these platforms enable public hosting of data with very low thresholds regarding *metadata* documentation and data standardisation. As a result, published datasets greatly vary in structure and nomenclature. 
...
As it stands, the decentralisation and the lack of data standardisation of data in online repositories renders the compilation of data into larger collections inefficient and reduces the potential of many published datasets to be reused and combined into broad synthesis analysis.
...
In the past two decades, many distributed trait datasets have been aggregated and harmonised into greater collections with particular taxonomic or regional focus [e.g. @klotz02; @iversen17; @kleyer08; @myhrvold15; @kissling14; @jones09; @oliveira17, see Supplementary Material A, table A1].<!--[@wright04; ] -->
...
Just as the individual datasets described above, many of these databases are published as static data tables on journal websites or open-access file hosting platforms and may be updated irregularly.
...
Concerning organism traits, with the digitisation efforts that are currently undertaken in many museum collections [@vollmar10; @blagoderov12], supported by citizen science crowd-sourcing (e.g. www.markmybird.org), data on body measurements are likely to skyrocket in the near future. 
For example, the VertNet database compiled and harmonised large quantities of vertebrate trait data with the aim of mobilising measurements from collections [@guralnick16]. The resulting data are published as versioned data tables which are updated as new data sources become available (vertnet.org). 
...
For animals, a single unified platform and harmonising scheme for animal trait data is still lacking. The reason for this may be that harmonising trait data on animals, which span multiple trophic levels and possess diverse body plans, is a more complex task than for plants [@moretti17]. 
Nonetheless, initiatives for particular groups of animals do exist: for instance, the BETSI database collects traits on soil invertebrates [http://betsi.cesab.org/; @pey14], the Carabids.org web portal collects traits of carabid beetles (http://www.carabids.org/), or the Coral Trait Database compiles ecological data on coral observations [@madin16].

The role of database initiatives in standardising data and making them more accessible is paramount. 
The trait-data platforms incentivise data submissions by offering increased data visibility and usage, while providing data-use policies that secure author attribution and, potentially, co-authorship. 
However, the upload of data to these centralised databases does often not fulfill all of the criteria that are increasingly demanded by journals and funding agencies for primary data publication with long-term availability, citable DOIs and no access restrictions. 
...
The distinction of thesauri and ontologies is not truly binary. Rather they mark idealised ends of a spectrum. While thesauri may contain defined relations between terms within the standard, ontologies relate most terms to other defined concepts, and also link those to other standards. 
...
These relationships between a trait observation and an individual organism as an occurrence of a particular taxon have been formalised in the schema for biological collection records [ABCD Schema; @holetschek12] and the Darwin Core Standard for biodiversity data [DwC; @wieczorek12]. 
...
For example, the Global Biodiversity Information Facility (GBIF, www.gbif.org) applies the terms described therein.
...
TraitBank invites data submissions to the EOL database in a structured Darwin Core Archive [DwC-A, @robertson09a], a zip-file with annotated text-files that is also preferred for observation data in GBIF [@gbif17,  http://tools.gbif.org/dwca-assistant/]. 
...
By filling this gap, data-brokering services [e.g. the German Federation for Biological Data; gfbio.org; @diepenbroek14] or data management systems for scientific projects (e.g. KNB and its open-source database back-end Metacat, https://knb.ecoinformatics.org/; Diversity Workbench, www.diversityworkbench.net; BExIS, http://bexis2.uni-jena.de/) are likely to gain importance. These services simplify and direct the standardised upload of research data and descriptive metadata into reliable and interlinked data infrastructures. One goal of such initiatives is to provide standardisation of data and facilitate data usage, for instance by mapping unambiguous terminologies and ontologies for biodiversity data, and clarify conditions of data re-use. Another solution for data-users to access trait data in a structured way is offered by FENNEC [@ankenbrand18], an online tool or self-hosted service to extract trait information from multiple sources for a target species community.  <!-- AG:  EDIT Platform for Cybertaxonomy (cybertaxonomy); NEON Dataportal http://data.neonscience.org/home;   -->
...
On the other end, public repositories on file-hosting services offer the publication of scientific primary data according to current funding and journal criteria, attracting a wealth of heterogeneous trait datasets. Due to the low thresholds concerning data standardisation, compilation of these distributed data sets is currently laborious. 
The goal must be to better integrate these distributed data into the global biodiversity data-management ecosystem by raising awareness for data standardisation, accessibility and re-useability on the side of data providers. By establishing this as a best practice in trait-based research, trait-data will eventually fullfill the FAIR guiding principles for scientific data [@wilkinson16]. 
In the following sections, we propose the development of vocabularies that lower the thresholds and offer high pay-off in the visibility and reuse of published data. 
