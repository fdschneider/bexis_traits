...
author: Florian D. Schneider, Malte Jochum, Gaetane LeProvost, Andreas
  Ostrowski, Caterina Penone, Nadja K. Simons
csl: amnat.csl
output:
...
Functional traits are phenotypic characteristics that are related to the fitness and performance of an organism [@mcgill06d; @violle07] and have been used to describe the role of species in ecosystems or their ability to persist under certain environmental conditions (@refs).

Traits have proven useful for bridging individual level physiology and behaviour to processes at the ecosystem scale and patterns of biodiversity at the global level [@diaz13]. Comparative study of traits informs researchers about physiological and evolutionary trade-offs, and helps identifying economic strategies of organisms beyond taxonomy [@menezes10; @forister15]. Inferring an unknown functional role or ecological function of individuals from their apparent features [@duarte11; @fontaine05] is a promising venue to bypass taxonomic impediment, i.e. the fact that a majority of species are yet undescribed and little is known of their interactions with the environment and other organisms. For instance, defense traits and predator feeding traits have co-evolved, just as pollinator and flower traits. This 'trait-matching' enables the inferrence of ecological interactions from observed features of live specimens which may be used to inform ecological networks [@munoz17; @weiner14] and parameterize ecological population-dynamic models [@refs]. 

On the level of species assemblages, community weighted means or variance of functional traits [@laliberte10a] or multivariate measures of 'functional diversity' [@petchey09; @villeger08a] are used to understand changes in functional composition or the loss of functional diversity in consequence to global change or local anthropogenic land use [environmental filtering; @jennings09; @gossner16; @simons16;@hopfenmuller14; @mouillot13;  @vandewalle10; @borschig13]. Since traits are closely related to ecological mechanisms, such patterns in functional community composition are promising versatile statistical tools to assess the current state of an ecosystem [ @moretti13; @kuhsel15;  @gamez-virues15; @birkhofer14] or of ecosystem services [@wood15b; @bello10; @diaz07; @lavorel12; @allan15].  Due to the abovementioned trait-matching, these analyses are also a promising venue for a multitrophic community ecology aiming to unravel dependencies and synchrony in trait spaces between interacting functional groups [@soliveres16].
...
Despite the obvious benefit, an integration of several studies across taxa, trophic levels, or organisational scales remains difficult because of different organismal, methodological, and functional focus, which renders data on traits heterogeneous. 
...
Most prominently, plant traits from many different sources -- including the extensive LEDA traitbase for the Northwest-European flora [@kleyer08] -- have been collated into the quickly growing TRY database [@kattge11].  Other more specialised databases for plant traits exist, for instance flower traits [BiolFlor for German wild flowers, @klotz02] and or root measurements [Fine-root Ecology Database; @iversen17]. Traits on crop plants have been collected in agronomy databases like Planteome and BetyDB [https://www.betydb.org/; @lebauer17]. Trait-data for lower plants are less completely assessed. <!-- TRY collates trait data from other published databases with individually owned datasets, provides an extended access management, data query services and tracking of data usage in studies and secondary publications. The great achievement of TRY is to harmonize these data into a unified scheme and provide a single platform of global coverage, which takes a huge effort to attract data submissions and map them into a common scheme. Repeating this exercise for other organism  -->
...
In the animal kingdom, specialised trait-databases have been created that cover different organism groups, interaction types, or ecosystems. For instance, The BETSI database collects traits on soil invertebrates (http://betsi.cesab.org/), or the Carabids.org web portal (http://www.carabids.org/). Besides these databases, an unknown plethora of single datasets has been published along with article publications over the past decades[e.g., @gossner15, @gossner15c, @renner14]. However, a unified platform and harmonizing scheme for the datasets is still lacking. It has been pointed out that data on animals, covering multiple trophic levels and having evolved very different body plans will not be a simple task and go far beyond the efforts produced for plant traits. [@refs]
...
## Need for ontologies for data standardisation

Public research funding agencies increasingly demand the publication of data without access restrictions [@swan12; @allison15]. However, although the number of trait-based studies grows constantly, not all scientific databases for functional traits are compliant with this demand for open data. <!--The training on compliance with open data standards within the community of environmental sciences can still be much improved [@schmidt16].--> Open-access publications in low-threshold data repositories such as Figshare, datadryad, Researchgate or Zenodo.org are gaining importance and foster a decentralised data hosting with low expectations regarding data standardization and documentation. Given this prospect, there will be no lack of data, but a lack of data standardization hampering efficient future use of published datasets.
...
\caption{Formats used for trait datasets. Identifiers relate the measurement to other layers of information on the specimen, the taxon or the trait measured. a) taxon-level traits are often published as a compiled species $\times$ traits matrix; b) measured data report measurements on the specimen-level, i.e. one physical occurence of a taxon; c) a measurement longtable is a well defined and tidy data format, reporting one single measurement per row and relating it to an occurrence and taxon via unambiguous identifiers.   }

\end{figure}

## common terms for trait data

In the subsequent sections we propose a set of terms that describe the columns composing a trait dataset compliant with the EST, starting with the ones required for our minimal definition of 'core' trait-datasets proposed above (Table 1).
...
Another set of terms is intended to facilitate the definition of traits in ontologies or a thesaurus of traits, consistent with semantic web standards (@ref, see below for further detail).  The full list of terms  can be accessed at https://ecologicaltraitdata.github.io/TraitDataStandard/ (will be moved to GFBio Terminology Server before publication).

A dataset that applies these terms should relate to the EST in its metadata. Since the traitdata standard is a resource under open community development, the metadata also should refer to a version number.  This is best warranted by providing the full URI of the applied  traitdata standard in the metadata (e.g. http://terminologies.gfbio.de/traitdatastandard/v1.0 !invalid URL!). 
...
A similar approach to standardised trait names (`traitNameStd`) and harmonised reported values (`traitValueStd` and `traitUnitStd`) in compliance with published ontologies is advised. 
...
Resources for semantic ontologies exist for plant traits [e.g. TOP Plant Trait Ontology , @garnier17], soil invertebrates [T-SITA, @pey14], or biomedical traits of vertebrates [Vertebrate Trait Ontology, @park13]. However, for most organism groups and research contexts we attest a deficit of public ontologies with publicly accessible URIs. 

### Defining trait thesauri and ontologies
...
Relating the trait definition to methodological definitions on trait measurements help standardize the physical process of measurements. The Handbook for Measurement of tratits [@moretti17a] is a model example of such a methodological standardization approach.
...
This identifier is usually dataset-specific and can be defined by the author and described with further detail in a separate data table using the columns provided in the occurrence extension detailed below. Some data-types may benefit from existing global identifiers for occurrence data, e.g. a GBIF URI or a museum collection code referencing the precise specimen at which the measuremetn was taken and that is publicly available.
...
Additionally, information about the authorship and ownership of the data and the terms of use should be considered. 
...
Many terms of this section refine terms of the Darwin Core Standard and its own MeasurementOrFact and [Occurrence](http://tools.gbif.org/dwca-validator/extension.do?id=http://rs.tdwg.org/dwc/terms/Occurrence#Occurrence) extensions. The terms may be applied as columns in a supplementary data sheet or along with the core data described above. Having separate data sheets linked by identifiers has the advantage of tidy datasets, avoiding duplication of information (@wickham14a). They furthermore can easily be packaged into Darwin Core Archives which are ideal for structured data (@parr16, Darwin core …).  If the additional information provided is limited to few types, the extension columns can also be added to the core data table.  
...
To access data from public data-bases, the R-package 'traits' [@chamberlain17] contains a set of functions to extract trait data via the open API interfaces of Birdlife, EOL TraitBank or BetyDB. We provide a script (see online appendix and R-package vignette) to demonstrates the harmonization of these different data sources. 
...
Furthermore, the tools provided here can be applied to harmonise and combine data from heterogeneous sources. We hope this aids research on the response of traits to environmental and anthropogenic drivers, synthesis on trait patterns on all ecological scales [@messier10], as well as multi-trophic trait analysis [@soliveres16; @allan15]. 
...
Thanks to all respondents to an internal online survey on trait data for the Biodiversity Exploratories project  and to Matthias Biber and Diana Bowler  for comments on the trait data standard and R-package. 
...
<!-- Malte: I just started adding personal funding sources here. -->
