...
author: Florian D. Schneider, Malte Jochum, Gaetane LeProvost, Nadja K. Simons, Andreas
  Ostrowski, Caterina Penone
csl: amnat.csl
output:
...
1.	The assessment and analysis of functional traits has been established in all domains of ecological research promising great advancements for biodiversity and climate research. 
2.	Trait data are stored in heterogeneous formats, measured at different resolutions and using different methods, comprising literature and measured data, and focused on a variety of research questions and organism groups. 
3.	To harmonize and standardize trait data, we present a first version of an ecological trait-data standard for storing and sharing functional trait data.
4.	We further provide an R-package that assists in transferring own data structures into the data standard and to harmonize and merge trait datasets from heterogeneous sources. 
5.	With these decentralized tools for traitdata management, we intend to facilitate the exchange and analysis of trait data for ecological research. 
...
Most prominently, plant traits from many different sources -- including the extensive LEDA traitbase for the Northwest-European flora [@kleyer08] -- have been collated into the quickly growing TRY database [@kattge11].  Other more specialised databases for plant traits exist, for instance flower traits [BiolFlor for German wild flowers, @klotz02] and or root measurements [Fine-root Ecology Database; @iversen17]. Traits on crop plants have been collected in agronomy databases like Planteome and BetyDB [https://www.betydb.org/; @lebaur17]. Trait-data for lower plants are less completely assessed. <!-- TRY collates trait data from other published databases with individually owned datasets, provides an extended access management, data query services and tracking of data usage in studies and secondary publications. The great achievement of TRY is to harmonize these data into a unified scheme and provide a single platform of global coverage, which takes a huge effort to attract data submissions and map them into a common scheme. Repeating this exercise for other organism  -->

Microbial ecology also makes great use of trait-based approaches to assess genomic function and describe functional diversity at the community level [@krause14].<!-- more on microbial databases --> 

In the animal kingdom, specialised trait-databases have been created that cover different organism groups, interaction types, or ecosystems. For instance, The BETSI database collects traits on soil invertebrates (http://betsi.cesab.org/), or the Carabids.org web portal (http://www.carabids.org/). Besides these databases, an unknown plethora of single datasets has been published along with article publications over the past decades[e.g., @Gossner2015SData, @Gossner2015Ecology, @Renner2014]. However, a unified platform and harmonizing scheme for the datasets is still lacking. It has been pointed out that data on animals, covering multiple trophic levels and having evolved very different body plans will not be a simple task and go far beyond the efforts produced for plant traits. [refs]
...
Relating the trait definition to methodological definitions on trait measurements help standardize the physical process of measurements. The Handbook for Measurement of tratits [@moretti17] is a model example of such a methodological approach.
