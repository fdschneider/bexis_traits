...
We encourage publishing trait thesauri developed for the own project context as public resources, e.g. on OBO Foundry, the GFBio Terminology service, or on other stable web services. 
...
We define a set of terms that allows to relate trait data to authorship and define a bibliographic reference and license model. 
...
This is usually the case if the occurrences are externally documented, for instance as specimens from a museum. This also applies if the data are stored in additional data tables. 
...
- the `measurementOrFact` extension takes information at the level of single measurements or reported facts, such as the original literature from where the value is cited, the method of measurement or statistical method of aggregation. 
...
Some data-types may directly refer to existing global identifiers for occurrence data, e.g. a GBIF URI or a museum collection code referencing the precise specimen at which the measurement was taken and that is publicly available.
...
- preparing of own trait datasets for publication on low-threshold fileservers and project databases, and
- harmonising trait datasets from different sources by moulding them into a unified format. 

The key function of the package is `as.traitdata()` which moulds a species-trait-matrix or occurrence table into a measurement longtable format. It also maps column names into terms provided in the trait data standard. 
...
Scientific taxon names are matched automatically to the GBIF Backbone Taxonomy and added to the core data by calling the function `standardize.taxonomy()`. 
...
Several datasets that have been produced by these functions can easily be appended using the `rbind()` function of R base. 

To merge datasets with additional information on the occurrence or measurement level, secondary data tables can be added as columns of the core dataset according to a unique identfier using the function `merge()`. This enables an easy handling of data sources that have been stored in a Darwin Core Archive database format. 

For a detailed description on how to use the package see the package documentation website. 
...
Since the intention of the package is to facilitate upload of own data to hosting services but also simplify the access to published trait data, the package offers direct access to datasets listed in our online spreadsheet that have been published in the public domain or under creative commons licenses [@gossner15; @gossner15c; @ricklefs17; @vanderplas17]. 
