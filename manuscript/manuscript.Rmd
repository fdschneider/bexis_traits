...
Open-access publications in low-threshold data repositories such as Figshare, dataDryad, Researchgate or Zenodo.org are gaining importance and foster a decentralised data hosting with low expectations regarding data standardization and documentation. Although open for manual download, data on those platforms might be stored in proprietary (e.g. .xlsx) or binary (e.g. .pdf) data formats which make a programmatical extraction tedious or impossible. Since the platforms are open for any type of scientific data, no data standards are enforced upon the uploaded material. Given this prospect, there will be no lack of data, but a lack of data standardization hampering efficient future use of published datasets.
...
However, the Terms of the Darwin Core Standard and available Extensions, TraitBank and VertNet, are responses to a demand in Biodiversity Data Management for structured input from the research communities. The practical importance of these frameworks for data providers and data users is less obvious and often not appropriately considered. To date, the existing trait data standards do not cover all of the aspects required in the various use-cases of trait-based research. <!-- examples for blind spots --> 

# Proposing an ecological traitdata standard 

With this paper, we propose a roadmap towards a comprehensive Ecological Traitdata Standard (EST) for general application in trait-based research, particularly building upon the work of @parr16, @kattge11, and @garnier17, and drawing on the combined expertise of empirical biodiversity researchers (data providers), biodiversity synthesis researchers (data users), and biodiversity informatics researchers (data managers).  

We define a versatile, minimal data structure, as well as a scheme of defined column names that captures the different degrees of resolution and measurement detail for multiple use cases of trait data. We also provide computational tools in an R package to help users transfer their original data into the proposed scheme.

With this framework, we build upon the structure proposed for EOL TraitBank [@parr16], which applies and expands terms of the Darwin Core standard [DWC; @wieczorek12]. We further expand the definitions of these terms for the use case of trait data and add terms which cover the special demands of trait-based research, while maintaining semantic compatibility with existing initatives. 

## Common structure of trait datasets

As a minimum consensus, trait datasets may be defined as follows: 
A trait-dataset contains quantitative measurements or qualitative facts (i.e. trait values) about physical phenotypic characteristics of fitness, behaviour or performance (i.e. traits) observed on an individual or parts of an individual (i.e. an entity) that can be assigned to a biological taxon (i.e. a species). The entity or observation (i.e. the occurrence) to which the reported measurement or fact applies may differ in resolution -- depending on the scientific question -- and could be a subsample or bodypart, an individual specimen, an entire species or higher-level taxon.  
...
Computationally more effective and allowing for highest flexibility is the storage of traitdata in 'tidied' long-table formats [@wickham14a], where each row is reserved for a single measurement or fact of a specific trait, referenced to a single occurrence (i.e. a specimen) which is assigned to a specific taxon. This allows for storing repeated measurements on a single individual by linking the data of separate rows via a unique specimen ID. Also, multivariate trait measurements can be recorded in this format by linking multiple rows via a unique measurement ID.  Longtable dataset purport multiple advantages for the data manipulation (e.g. filtering, subsetting and aggregating data), visualization (e.g. plot measured values by factor variable or taxon) and statistical modeling (e.g. ANOVA for testing difference of trait value by sex) [@wickham14a]. They further draw from a defined set of columns, which makes it easier to merge datasets. 
...
To comply with the EST, datasets must be reformatted to the longtable format. 
...
In the subsequent sections we propose a set of terms that describe the columns composing a trait dataset, starting with the ones required for our minimal definition of 'core' trait-datasets proposed above.
...
A second set of terms is intended to facilitate the definition of traits in ontologies or a thesaurus of traits, consistent with semantic web standards (@ref, see below for further detail). 
The full glossary of terms can be accessed at https://ecologicaltraitdata.github.io/TraitDataStandard/ (will be moved to GFBio Terminology Server before publication). Were possible, we relate these terms to existing terms in ontologies for biological data, like the Glossary of EOL (http://eol.org/info/516) and Darwin Core ().

A dataset that applies these terms should refer to the data standard in its metadata. Since the traitdata standard is a resource under open community development, this reference also should include the version number. This is best warranted by providing the full URI of the applied  traitdata standard in the metadata (e.g. http://terminologies.gfbio.de/traitdatastandard/v1.0 !invalid URL!). 
...
Keeping a reference to a kingdom and class (using field `kingdom`, `class` as described by Darwin Core Standard and suggested for EOL Traitbank) will avoid ambiguities with taxonomic homonyms. 
...
Similar as for `taxonID`, the field `traitID` should refer to globally unique identifiers (URI) of trait definitions or a measurement methodology. 
...
We encourage hosting such semantic web resources developed for own projects on public repositories like OBO Foundry, Github or the GFBio Terminology service. We provide an example thesaurus, that has been developed to capture traits assessed within the German Biodiversity Exploratories project on Github (). 
...
<!-- 
(We conceptualize trait-datasets to  not include information about a species' interaction partners/associations of an organism, since they are -- to our understanding -- not within the scope of functional traits. However, such network data may be of interest to be combined with trait data. ) <!-- Malte: which can only be achieved if both types of datasets have been related to an ontology of species names. --> <!--Nadja: I don't think that we need to mention this. If you think that this is important, it should be moved more to the beginning, e.g. within the section "Common terms for trait data"-->

## Extensions for additional data layers

Beyond measurement units or taxon rank resolution, further information might be available that are not recorded in the raw data table, but are related to the trait type, the taxon, the individual or specimen, or to the reported fact, measurement or sampling event. 

In most cases those information would be kept in seperate data sheets of a spreadsheet document, like for instance the place were a specimen has been sampled, the measurement method or statistical precision of the reported value, or the literature source from where a species value has been cited. In this case, a unique identifier might link to this other datasheet, encoding each individual occurrence of a species (`occurrenceID`), single measurements or reported facts (`measurementID`), locations of sampling (`locationID`) and sampling campaigns (`eventID`). 
...
Many terms of this section refine terms of the Darwin Core Standard and their own [MeasurementOrFact](http://rs.tdwg.org/dwc/terms/index.htm#measureindex) and [Occurrence](http://rs.tdwg.org/dwc/terms/index.htm#Occurrence) extensions  and EOL Trait Bank use of those terms. 
...
The terms may be applied as columns in a supplementary data sheet or along with the core data described above. Having separate data sheets linked by identifiers has the advantage of tidy datasets, avoiding duplication of information (@wickham14a). They furthermore can easily be packaged into Darwin Core Archives which are ideal for structured data (@parr16, Darwin core ...).  In that case, the traitdata table should at least contain a column with the respective identifier that directs to the covariate datasheet. 

However, it may be safer to add these information directly as own columns of the data table to enable an analysis of cofactors and correlations further down the road. This way, if datasets of different source are merged, the information is readily available without the risk of breaking the reference to an external datasheet. 

<!-- Nadja: can those last two paragraphs be merged with the second paragraph? I feel like we are repeating ourselves here. -->
...
The `standardize.traits()` function matches user-provided trait names onto public trait ontologies (for now using GBIF Backbone Taxonomy) and links it to a trait ontology or a user-provided table of trait definitions, i.e. a thesaurus of traits. The same function harmonises trait values into target units and legit factor levels. 
...
Thanks to all respondents to an internal online survey on trait data for the Biodiversity Exploratories project and to Matthias Biber, Diana Bowler, Klaus Birkhofer, Catrin Westphal, Runa Boeddinghaus for comments on the manuscript, the trait data standard and the R-package. 
