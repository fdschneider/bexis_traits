...
3. In order to facilitate the standardisation and harmonisation of distributed trait datasets, we propose a standardised vocabulary to be used for storing and sharing ecological trait data. 
4. This vocabulary allows trait datasets from heterogeneous sources to be merged more easily into databases, and facilitates the creation of data interfaces between software tools for trait-data handling and analysis. 
5. By enabling decentralised trait-data standardisation, we intend to facilitate the exchange and analysis of trait data for a broader ecological research community and enable global syntheses of traits across a wide range of taxa and ecosystems. 
...
Many issues in trait-based research arise when compiling datasets from multiple sources. Data may differ in taxonomic nomenclature and resolution (e.g. reported on species level or aggregated on higher taxonomical orders), the scale and place of the study context, or the accurracy of the methodology applied in measurements. These differences are not always documented in the metadata accompanying a dataset. 
...
<!--These databases are then published as static data tables (i.e. product of a one-time effort as it is the case of AmphiBIO), or in the form of sophisticated online portals, which attract data submissions from a focal community of researchers and provide research networks for consensus building in data resolution and measurement methods (e.g. TRY, www.try-db.org). Furthermore,--> These initiatives map heterogeneous data into a common scheme and, importantly, may also offer access control and data usage policies. As such, they protect the rights of the original data providers while simplifying data queries for synthesis researchers. 
...
Concerning organism traits, with the digitisation efforts that are currently undertaken in many museum collections [@vollmar10; @blagoderov12], supported by citizen science crowd-sourcing (e.g. www.markmybird.org), data on body measurements are likely to grow exponentially in the near future. 
...
For animals, a single unified platform and harmonising scheme for animal trait data is still lacking. The reason for this may be that harmonising trait data on animals, which span multiple trophic levels and possess diverse body plans, is a more complex task than for plants [@moretti17]. Nonetheless, initiatives for particular groups of animals do exist, such as the BETSI database collects traits on soil invertebrates [http://betsi.cesab.org/; @pey14], and the Carabids.org web portal collects traits of carabid beetles (http://www.carabids.org/). 

The platforms incentivise data submissions by offering increased data visibility and usage, while providing data use policies that secure author attribution and potentially co-authorship. 
However, the upload of data to these centralised databases does currently not fulfill the criteria demanded by journals and funding agencies for primary data publication with long-term availability, citable DOIs and no access restrictions. With data being publicly available, the role of database initiatives in standardising data and making them more accessible is paramount. 
...
We propose the development of tools and vocabularies that impose low thresholds and offer high pay-off in the visibility and interconnectedness of published data. 
...
Here, the use of identifiers ('IDs') for the individual measurement observations ('measurementID'), specimens ('occurrenceID'), sampling events ('eventID'), or taxa ('taxonID') is key to map two-dimensional data onto the structure of relational databases. 
...
The vocabulary proposed is intended to form the foundations of a standard exchange format that can be expanded and revised by the wider community of researchers using trait-based approaches in ecology.
<!--
...
These information may be useful for future analysis of the causal reasons of trait variation and should always be published along with the core data. For this case, we offer three extensions of the core vocabulary ("Taxon", "Measurement or Fact", and "Occurrence"), which may simply be added as extra columns to the core dataset (see below for best practice when publishing primary data).
...
Some of the terms expand and refine terms of the Darwin Core Extensions (http://rs.tdwg.org/dwc/terms/) or Glossary of EOL (http://eol.org/info/516). Wherever our glossary refines or duplicates existing terms, we indicate this in the fields 'refines' or 'identical', respectively.
...
Wherever possible, the column `traitID` should point to a publicly available, unambiguous trait definition in a published ontology. 
...
While it is clear, that a single vocabulary standard is not able to fullfill the needs of all research fields working with traits, we believe that it captures the a common core concept of trait data. 
Consensus building on how traits are to be used and evaluated is currently under way in several fields with their taxonomic focus and project-specific questions. While these community discussions on trait definitions and measurement practice is leading to a better quality of data, naturally, the heterogeneity in trait-data quality and detail across fields will remain. 
...
The R-package 'traitdataform' [@schneider18] is assisting the conversion of own data into compliant formats. 
...
The package 'NatDB' [@pearse] collates distributed trait data and harmonizes them into a unified format. 
...
Even further downstream, machine-readable, ontology-based data will ease the application of big-data mining and machine-learning techniques. 
Without clearly defined terms and concepts, computational handling of large amounts of trait-data will be impossible. 
...
The Appendix A contains a static excerpt of the living spreadsheet on existing trait datasets and databases, which can be found at https://goo.gl/QxzfHy. 
